        ...
        <div className="min-h-screen bg-slate-50 dark:bg-black p-4 sm:p-6 lg:p-12 font-sans">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
                    <div>
                        ...
                        <h1 className="text-3xl sm:text-4xl font-black text-slate-900 dark:text-white tracking-tight">
                            Student <span className="text-slate-400 dark:text-white/30 font-light">Management</span>
                        </h1>
                        <p className="text-slate-500 dark:text-white/40 font-medium mt-1">Add and manage examinees for your institution</p>
                    </div>
                    {(user?.role === 'owner' || user?.role === 'instructor') && (
                        <button
                            onClick={() => setShowAddModal(true)}
                            className="flex items-center gap-2 px-6 py-3.5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-2xl hover:bg-slate-800 dark:hover:bg-slate-100 transition-all font-bold shadow-lg shadow-slate-900/20 active:scale-95"
                        >
                            <UserPlus size={18} />
                            Add Student
                        </button>
                            ...
                            <table className="w-full">
                                ...
                                <tbody className="divide-y divide-slate-100 dark:divide-white/[0.04]">
                                    {students.length === 0 ? (
                                        <tr>
                                            <td colSpan="5" className="py-24 text-center">
                                                <div className="w-20 h-20 bg-slate-50 dark:bg-white/[0.03] rounded-3xl flex items-center justify-center mx-auto mb-6 border border-slate-200 dark:border-white/[0.06]">
                                                    <Users size={32} className="text-slate-300 dark:text-white/20" />
                                                </div>
                                                <h3 className="text-xl font-black text-slate-900 dark:text-white mb-2">No Students Found</h3>
                                        ...
                                        <tr key={student._id} className="hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors">
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <p className="font-bold text-slate-900 dark:text-white/90 text-sm">{student.name}</p>
                                                        <p className="text-xs text-slate-500 dark:text-white/40">{student.email}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                                <span className="text-xs font-black text-slate-500 dark:text-white/40 bg-slate-100 dark:bg-white/[0.05] border border-slate-200 dark:border-white/[0.06] px-3 py-1 rounded-full">
            ...
            {showAddModal && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 transition-all duration-300">
                    ...
                    <div className="relative w-full max-w-md bg-white dark:bg-[#0a0a0a] rounded-[32px] border border-slate-200 dark:border-white/[0.08] overflow-hidden transition-all duration-500 p-8">
                        <div className="flex items-center justify-between mb-8">
                            <div>
                                <h2 className="text-2xl font-black text-slate-900 dark:text-white">Add Student</h2>
                                <p className="text-sm text-slate-500 dark:text-white/40 mt-1">Credentials auto-generated and emailed</p>
                            </div>
                            <button onClick={() => setShowAddModal(false)} className="p-2 text-slate-500 dark:text-white/40 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/[0.06] rounded-xl transition-all">
                                <X className="w-5 h-5" />
                            </button>
                        </div>
                        ...
                        <form onSubmit={handleAddStudent} className="space-y-5">
                            <div>
                                <label className="block text-xs font-black text-slate-500 dark:text-white/40 uppercase tracking-widest mb-2">Full Name</label>
                                <input
                                    type="text" required
                                    value={newStudent.name}
                                    onChange={e => setNewStudent({ ...newStudent, name: e.target.value })}
                                    placeholder="e.g. John Doe"
                                    className="w-full px-4 py-3.5 bg-slate-50 dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.08] rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500/50 outline-none text-sm font-medium text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-white/30 transition-all"
                                />
                            </div>
                            <div>
                                <label className="block text-xs font-black text-slate-500 dark:text-white/40 uppercase tracking-widest mb-2">Email Address</label>
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full pl-11 pr-4 py-3.5 bg-slate-50 dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.08] rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500/50 outline-none text-sm font-medium text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-white/30 transition-all"
                                    />
                                </div>
                            </div>

                            <div className="bg-indigo-50 dark:bg-indigo-500/10 p-4 rounded-xl border border-indigo-100 dark:border-indigo-500/20 flex gap-3.5 items-start">
                            ...
                            <div className="flex gap-3 pt-2">
                                <button type="button" onClick={() => setShowAddModal(false)}
                                    className="flex-1 px-6 py-3.5 bg-slate-100 dark:bg-white/[0.05] hover:bg-slate-200 dark:hover:bg-white/[0.09] rounded-xl text-slate-600 dark:text-white/60 hover:text-slate-900 dark:hover:text-white font-bold text-xs uppercase tracking-widest transition-all active:scale-95 border border-slate-200 dark:border-white/[0.07]">
                                    Cancel
                                </button>
                                <button type="submit" disabled={submitting}
                                    className="flex-1 px-6 py-3.5 bg-slate-900 dark:bg-white/10 hover:bg-slate-800 dark:hover:bg-white/[0.16] text-white dark:text-white rounded-xl font-bold text-xs uppercase tracking-widest transition-all active:scale-95 border border-transparent dark:border-white/10 flex items-center justify-center gap-2 disabled:opacity-70">
                                    {submitting ? (
                                        <div className="w-4 h-4 border-2 border-slate-500/30 dark:border-white/30 border-t-slate-900 dark:border-t-white rounded-full animate-spin" />
                                    ) : (
                                        'Add Student'
                                    )}
                                </button>
