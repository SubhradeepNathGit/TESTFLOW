        ...
        <div className="min-h-screen bg-[#F8F9FD] p-6 lg:p-10">
            <div className="max-w-7xl mx-auto">
                ...
                <motion.div
                    ...
                    className="relative bg-white border border-slate-100 rounded-[32px] p-8 lg:p-10 mb-8 overflow-hidden shadow-sm flex items-center justify-between"
                >
                    <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-50/50 rounded-full blur-3xl -translate-y-1/2 translate-x-1/4 pointer-events-none" />
                    <div className="relative z-10 flex items-center gap-6 lg:gap-8">
                        <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-full border border-slate-100 shadow-sm flex items-center justify-center bg-indigo-50 text-indigo-600 text-3xl font-black shrink-0 overflow-hidden relative">
                            {user?.profileImage ? (
                                <img src={user.profileImage.startsWith('http') ? user.profileImage : `http://localhost:3006/${user.profileImage}`} alt="Profile" className="w-full h-full object-cover rounded-full" />
                            ) : (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
                    {kpis.map((kpi, i) => (
                        <motion.div
                            ...
                            initial="hidden"
                            animate="show"
                            className="bg-white p-5 rounded-[24px] border border-slate-100 shadow-sm flex items-center gap-4"
                        >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2">
                        <div className="flex items-center gap-3 mb-5">
                            ...
                            <span className="ml-auto text-[10px] font-black text-slate-400 bg-slate-50 border border-slate-100 px-3 py-1.5 rounded-full uppercase tracking-widest">
                                {tests.length} total
                            </span>
                        </div>
                        <div className="grid gap-3">
                            {tests.length === 0 ? (
                                <div className="bg-white p-14 rounded-[28px] border border-slate-100 text-center">
                                    ...
                                    <p className="text-xs text-slate-300 mt-1">Your instructor hasn't published any tests.</p>
                                </div>
                            ) : tests.map((test, idx) => {
                                ...
                                return (
                                    <motion.div
                                        ...
                                        initial="hidden"
                                        animate="show"
                                        className={`bg-white p-5 rounded-[22px] border transition-all flex items-center justify-between gap-4 group ${
                                            isDone
                                                ? 'border-slate-100 shadow-sm'
                                                : 'border-slate-100 shadow-sm hover:shadow-md hover:border-indigo-100 cursor-pointer'
                                        }`}
                                    ...
                                    >
                                        <div className="flex items-center gap-4 min-w-0">
                                            ...
                                            <div className="min-w-0">
                                                <h3 className="text-sm font-bold text-slate-800 truncate">{test.title}</h3>
                    ...
                    <div>
                        <div className="flex items-center gap-3 mb-5">
                            ...
                            <h2 className="text-base font-bold text-slate-800">My Results</h2>
                        ...
                        <div className="space-y-3">
                            {validAttempts.length === 0 ? (
                                <div className="bg-white p-10 rounded-[28px] border border-slate-100 text-center">
                                    <FiTrendingUp size={32} className="text-slate-200 mx-auto mb-3" />
                                    <p className="text-slate-400 font-semibold text-sm">No attempts yet</p>
                                    <p className="text-xs text-slate-300 mt-1">Start a test to see your results.</p>
                                </div>
                                ...
                                return (
                                    <motion.div
                                        ...
                                        initial="hidden"
                                        animate="show"
                                        className="bg-white p-4 rounded-[20px] border border-slate-100 shadow-sm"
                                    >
                                        <div className="flex items-start justify-between gap-2 mb-2">
                                            <p className="font-bold text-slate-800 text-sm leading-tight truncate">{a.testId?.title || 'Test'}</p>
                                            <div className="text-right shrink-0">
                                                <p className="text-lg font-black text-slate-900 tabular-nums">{a.score}</p>
                            ...
                            <button
                                ...
                                className="w-full bg-indigo-600 text-white py-3.5 rounded-2xl font-bold text-sm hover:bg-indigo-700 transition-all flex items-center justify-center gap-2 mt-4 shadow-lg shadow-indigo-100 active:scale-95"
                            >
                                <FiAward size={15} />
